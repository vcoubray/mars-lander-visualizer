...
import fr.vco.genetic.algorithm.visualizer.codingame.toResult
import fr.vco.genetic.algorithm.visualizer.exceptions.AlreadyRunningException
...
class SimulationService(
    ...
    fun start(simulationSettings : SimulationSettings<*>): Int {
        ...
        thread {
            ...
            val runFunction = when (simulationSettings.globalSettings.limitType) {
                LimitType.TIME -> algo::runUntilTime
                LimitType.SCORE -> algo::runUntilScore
            }
            val duration = measureTimeMillis {
                runFunction(simulationSettings.globalSettings.limitValue) { generation ->
                    generations.add(
                        GenerationResult(
